...
if ! kubectl -n "$ns" get secret web-tls >/dev/null 2>&1; then
  if command -v openssl >/dev/null 2>&1; then
    echo "[setup] Generating self-signed cert via openssl"
    tmpd=$(mktemp -d)
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
      -subj "/CN=gateway.web.k8s.local" \
      -keyout "$tmpd/tls.key" -out "$tmpd/tls.crt" >/dev/null 2>&1 || true
    kubectl -n "$ns" create secret tls web-tls --key "$tmpd/tls.key" --cert "$tmpd/tls.crt" || true
    rm -rf "$tmpd"
  else
    echo "[setup] openssl not found; applying placeholder TLS secret"
...
echo "[setup] Applying Deployments, Services, and Ingress"
kubectl apply -f - <<'YAML'
apiVersion: apps/v1
kind: Deployment
metadata:
