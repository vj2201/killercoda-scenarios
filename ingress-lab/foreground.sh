...
for i in {1..90}; do
  if kubectl get ns echo-sound >/dev/null 2>&1 && \
     kubectl -n echo-sound get deploy echo-deployment >/dev/null 2>&1 && \
     kubectl get ingressclass nginx >/dev/null 2>&1; then
    setup_complete=true
    echo "   ✓ Setup complete"
    break
  fi
...
if [ "$setup_complete" = "false" ]; then
  ...
  echo "   kubectl get ns echo-sound"
  echo "   kubectl get deploy -n echo-sound"
